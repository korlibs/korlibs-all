...
import com.soywiz.korau.format.util.FloatDSP.vectorFmul
import com.soywiz.korau.format.util.FloatDSP.vectorFmulReverse
import com.soywiz.korau.format.util.SineWin
import com.soywiz.korau.format.util.SineWin.ff_sine_128
import com.soywiz.korau.format.util.SineWin.ff_sine_64
import kotlin.math.*
...
class Atrac3plusDsp {
	...
	fun powerCompensation(ctx: ChannelUnitContext, chIndex: Int, sp: FloatArray, rngIndex: Int, sb: Int) {
		...
		val g1 = channel1.gainData[sb]
		val g2 = channel1.gainDataPrev[sb]

		val gainLev = if (g1.numPoints > 0) 6 - g1.levCode[0] else 0

		for (i in 0 until g2.numPoints) {
			gcv = max(gcv, gainLev - (g2.levCode[i] - 6))
		}

		for (i in 0 until g1.numPoints) {
			gcv = max(gcv, 6 - g1.levCode[i])
		}

		val grpLev = pwc_levs[channel1.powerLevs[subband_to_powgrp[sb]]] / (1 shl gcv)
		...
		for (qu in subband_to_qu[sb] + (if (sb == 0) 2 else 0) until subband_to_qu[sb + 1]) {
			...
			val quLev = ff_atrac3p_sf_tab[channel.quSfIdx[qu]] * ff_atrac3p_mant_tab[channel.quWordlen[qu]] / (1 shl channel.quWordlen[qu]) * grpLev

			val dst = ff_atrac3p_qu_to_spec_pos[qu]
			val nsp = ff_atrac3p_qu_to_spec_pos[qu + 1] - ff_atrac3p_qu_to_spec_pos[qu]

			for (i in 0 until nsp) {
				sp[dst + i] += pwcsp[i] * quLev
			}
		}
	}

	fun imdct(mdctCtx: FFT, `in`: FloatArray, inOffset: Int, out: FloatArray, outOffset: Int, windId: Int, sb: Int) {
		if (sb and 1 != 0) {
			for (i in 0 until ATRAC3P_SUBBAND_SAMPLES / 2) {
				val tmp = `in`[inOffset + i]
		...
		if (windId and 2 != 0) { // 1st half: steep window
			out.fill(0f, outOffset, outOffset + 32)
		...
		if (windId and 1 != 0) { // 2nd half: steep window
			vectorFmulReverse(out, outOffset + 160, out, outOffset + 160, ff_sine_64, 0, 64)
			out.fill(0f, outOffset + 224, outOffset + 224 + 32)
	...
	private fun wavesSynth(synthParams: WaveSynthParams, wavesInfo: WavesData, envelope: WaveEnvelope, phaseShift: Boolean, regOffset: Int, out: FloatArray) {
		...
		while (wn < wavesInfo.numWavs) {
			...
			var pos = DEQUANT_PHASE(waveParam1.phaseIndex) - (regOffset xor 128) * inc and 2047

			// waveform generation
			for (i in 0..127) {
				out[i] += (sine_table[pos] * amp).toFloat()
				pos = pos + inc and 2047
			}
			wn++
			waveParam++
		}
	...
	fun ipqf(dctCtx: FFT, hist: ChannelUnitContext.IPQFChannelContext, `in`: FloatArray, out: FloatArray) {
		val idctIn = FloatArray(ATRAC3P_SUBBANDS)
		val idctOut = FloatArray(ATRAC3P_SUBBANDS)

		out.fill(0f, 0, ATRAC3P_FRAME_SAMPLES)

		for (s in 0 until ATRAC3P_SUBBAND_SAMPLES) {
			// pack up one sample from each subband
			for (sb in 0 until ATRAC3P_SUBBANDS) {
			...
			var posNow = hist.pos
			var posNext = mod23_lut[posNow + 2] // posNext = (posNow + 1) % 23

			for (t in 0 until ATRAC3P_PQF_FIR_LEN) {
				for (i in 0..7) {
	...
	companion object {
		private val ATRAC3P_MDCT_SIZE = ATRAC3P_SUBBAND_SAMPLES * 2
		private val sine_table = FloatArray(2048) ///< wave table
		private val hann_window = FloatArray(256) ///< Hann windowing function
		private val amp_sf_tab = FloatArray(64)   ///< scalefactors for quantized amplitudes
		...
		fun initWaveSynth() {
			...
			for (i in 0..2047) {
				sine_table[i] = sin(TWOPI * i / 2048).toFloat()
			}

			// generate Hann window
			for (i in 0..255) {
				hann_window[i] = ((1.0 - cos(TWOPI * i / 256)) * 0.5).toFloat()
			}

			// generate amplitude scalefactors table
			for (i in 0..63) {
				amp_sf_tab[i] = 2.0.pow((i - 3).toDouble() / 4.0).toFloat()
			}
		}
	}
}
