...
class KmlGlAndroid : KmlGl() {
	...
	override fun bufferData(target: Int, size: Int, data: KmlNativeBuffer, usage: Int): Unit =
		glBufferData(target, size, data.nioBuffer, usage)

	override fun bufferSubData(target: Int, offset: Int, size: Int, data: KmlNativeBuffer): Unit =
		glBufferSubData(target, offset, size, data.nioBuffer)
	...
	): Unit = glCompressedTexImage2D(target, level, internalformat, width, height, border, imageSize, data.nioBuffer)
	...
	override fun compressedTexSubImage2D(
		...
		glCompressedTexSubImage2D(target, level, xoffset, yoffset, width, height, format, imageSize, data.nioBuffer)
	...
	override fun deleteRenderbuffers(n: Int, items: KmlNativeBuffer): Unit =
		glDeleteRenderbuffers(n, items.nioIntBuffer)
	...
	override fun genFramebuffers(n: Int, framebuffers: KmlNativeBuffer): Unit =
		glGenFramebuffers(n, framebuffers.nioIntBuffer)

	override fun genRenderbuffers(n: Int, renderbuffers: KmlNativeBuffer): Unit =
		glGenRenderbuffers(n, renderbuffers.nioIntBuffer)
	...
	override fun getActiveAttrib(
		...
		val alen = IntArray(1);
		val asize = IntArray(1);
		val atype = IntArray(1);
		val aname = ByteArray(name.size); glGetActiveAttrib(
		program,
		index,
		bufSize,
		alen,
		...
		aname,
		0
	); length.setInt(0, alen[0]); size.setInt(0, asize[0]); type.setInt(
		0,
		atype[0]
	); name.putAsciiString(aname.toString(Charsets.US_ASCII))
	}
	...
	override fun getActiveUniform(
		...
		val asize = IntArray(1);
		val atype = IntArray(1);
		val aname = ByteArray(name.size); glGetActiveUniform(
		program,
		index,
		bufSize,
		alen,
	...
	): Unit = glGetAttachedShaders(program, maxCount, count.nioIntBuffer, shaders.nioIntBuffer)
	...
	): Unit = run { infoLog.putAsciiString(glGetProgramInfoLog(program)) }
	...
	override fun getShaderInfoLog(shader: Int, bufSize: Int, length: KmlNativeBuffer, infoLog: KmlNativeBuffer): Unit =
		run { infoLog.putAsciiString(glGetShaderInfoLog(shader)) }
	...
	override fun getShaderSource(shader: Int, bufSize: Int, length: KmlNativeBuffer, source: KmlNativeBuffer): Unit =
		run {
			val len = IntArray(1); glGetShaderiv(shader, GL_SHADER_SOURCE_LENGTH, len, 0);
			val src = ByteArray(len[0]); glGetShaderSource(
			shader,
			bufSize,
			len,
			0,
			src,
			0
		); source.putAsciiString(src.toString(Charsets.US_ASCII))
		}
	...
	override fun shaderBinary(
		...
		length: Int
	): Unit = glShaderBinary(count, shaders.nioIntBuffer, binaryformat, binary.nioBuffer, length)
	...
	override fun uniform1fv(location: Int, count: Int, value: KmlNativeBuffer): Unit =
		glUniform1fv(location, count, value.nioFloatBuffer)
	...
	override fun uniform1iv(location: Int, count: Int, value: KmlNativeBuffer): Unit =
		glUniform1iv(location, count, value.nioIntBuffer)
	...
	override fun uniform2fv(location: Int, count: Int, value: KmlNativeBuffer): Unit =
		glUniform2fv(location, count, value.nioFloatBuffer)
	...
	override fun uniform2iv(location: Int, count: Int, value: KmlNativeBuffer): Unit =
		glUniform2iv(location, count, value.nioIntBuffer)
	...
	override fun uniform3fv(location: Int, count: Int, value: KmlNativeBuffer): Unit =
		glUniform3fv(location, count, value.nioFloatBuffer)
	...
	override fun uniform3iv(location: Int, count: Int, value: KmlNativeBuffer): Unit =
		glUniform3iv(location, count, value.nioIntBuffer)
	...
	override fun uniform4fv(location: Int, count: Int, value: KmlNativeBuffer): Unit =
		glUniform4fv(location, count, value.nioFloatBuffer)
	...
	override fun vertexAttribPointer(
		...
		stride: Int,
		pointer: Int
	): Unit = glVertexAttribPointer(index, size, type, normalized, stride, pointer)
