...
fun buildActions(callback: ActionBuilder.() -> Unit): List<Action> =
	ActionBuilder().apply(callback).run { ActionBuilder.Building.run { buildList() } }

fun buildAction(callback: ActionBuilder.() -> Unit): Action =
	ActionBuilder().apply(callback).run { ActionBuilder.Building.run { build() } }
