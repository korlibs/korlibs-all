...
object DebugBitmapFont {
	var bmpFont: Job<BitmapFont>? = null
...
suspend fun Scene.getDebugBmpFontOnce(): BitmapFont {
	if (DebugBitmapFont.bmpFont == null) {
		DebugBitmapFont.bmpFont = launch {
			val tex = texture(DebugBitmapFont.DEBUG_FONT_BYTES)
			val fntAdvance = 7
			val fntWidth = 8
			val fntHeight = 8
			BitmapFont(tex, fntHeight, fntHeight, (0 until 256).map {
				val x = it % 16
				val y = it / 16
				BitmapFont.BmpChar(it, x * fntWidth, y * fntHeight, fntWidth, fntHeight, 0, 0, fntAdvance, 0, 0)
			}, listOf())
		}
	}
	return DebugBitmapFont.bmpFont!!.await()
}
