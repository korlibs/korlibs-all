...
class KmlGlJsCanvas(val canvas: HTMLCanvasElement) : KmlGl() {
	...
	private fun <T> T.alloc(): Int = run {
		if (this.asDynamic().id === undefined) {
			val index = freeList.removeAt(freeList.size - 1); items[index] = this; (this.asDynamic()).id =
					index; }; this.asDynamic().id.unsafeCast<Int>()
	}

	private fun <T> Int.get(): T = items[this].unsafeCast<T>()
	private fun <T> Int.free(): T =
		run { val out = items[this].unsafeCast<T>(); freeList += this; items[this] = null; out }
	...
	override fun bindAttribLocation(program: Int, index: Int, name: String): Unit =
		gl.bindAttribLocation(program.get(), index, name)
	...
	override fun bindRenderbuffer(target: Int, renderbuffer: Int): Unit =
		gl.bindRenderbuffer(target, renderbuffer.get())
	...
	): Unit = gl.compressedTexImage2D(target, level, internalformat, width, height, border, data.arrayByte)
	...
	override fun compressedTexSubImage2D(
		...
		imageSize: Int,
		data: KmlNativeBuffer
	): Unit = gl.compressedTexSubImage2D(target, level, xoffset, yoffset, width, height, format, data.arrayByte)
	...
	override fun copyTexImage2D(
		...
		height: Int,
		border: Int
	): Unit = gl.copyTexImage2D(target, level, internalformat, x, y, width, height, border)
	...
	override fun copyTexSubImage2D(
		...
		width: Int,
		height: Int
	): Unit = gl.copyTexSubImage2D(target, level, xoffset, yoffset, x, y, width, height)
	...
	override fun deleteBuffers(n: Int, items: KmlNativeBuffer): Unit =
		run { for (p in 0 until n) gl.deleteBuffer(items.arrayInt[p].free()) }

	override fun deleteFramebuffers(n: Int, items: KmlNativeBuffer): Unit =
		run { for (p in 0 until n) gl.deleteFramebuffer(items.arrayInt[p].free()) }
	...
	override fun deleteRenderbuffers(n: Int, items: KmlNativeBuffer): Unit =
		run { for (p in 0 until n) gl.deleteRenderbuffer(items.arrayInt[p].free()) }
	...
	override fun deleteTextures(n: Int, items: KmlNativeBuffer): Unit =
		run { for (p in 0 until n) gl.deleteTexture(items.arrayInt[p].free()) }
	...
	override fun framebufferRenderbuffer(
		...
		renderbuffer: Int
	): Unit = gl.framebufferRenderbuffer(target, attachment, renderbuffertarget, renderbuffer.get())

	override fun framebufferTexture2D(target: Int, attachment: Int, textarget: Int, texture: Int, level: Int): Unit =
		gl.framebufferTexture2D(target, attachment, textarget, texture.get(), level)

	override fun frontFace(mode: Int): Unit = gl.frontFace(mode)
	override fun genBuffers(n: Int, buffers: KmlNativeBuffer): Unit =
		run { for (p in 0 until n) buffers.arrayInt[p] = gl.createBuffer().alloc() }
	...
	override fun genFramebuffers(n: Int, framebuffers: KmlNativeBuffer): Unit =
		run { for (p in 0 until n) framebuffers.arrayInt[p] = gl.createFramebuffer().alloc() }

	override fun genRenderbuffers(n: Int, renderbuffers: KmlNativeBuffer): Unit =
		run { for (p in 0 until n) renderbuffers.arrayInt[p] = gl.createRenderbuffer().alloc() }

	override fun genTextures(n: Int, textures: KmlNativeBuffer): Unit =
		run { for (p in 0 until n) textures.arrayInt[p] = gl.createTexture().alloc() }
	...
	override fun getActiveAttrib(
		...
		val info = gl.getActiveAttrib(program.get(), index)!!; size.arrayInt[0] = info.size; type.arrayInt[0] =
			info.type; name.putAsciiString(info.name); length.arrayInt[0] = info.size + 1
	}
	...
	override fun getActiveUniform(
		...
		val info = gl.getActiveUniform(program.get(), index)!!; size.arrayInt[0] = info.size; type.arrayInt[0] =
			info.type; name.putAsciiString(info.name); length.arrayInt[0] = info.size + 1
	}
	...
	override fun getAttachedShaders(
		...
		val ashaders = gl.getAttachedShaders(program.get())!!; count.arrayInt[0] = ashaders.size; for (n in 0 until min(
		maxCount,
		ashaders.size
	)) shaders.arrayInt[n] = ashaders[n].asDynamic().id.unsafeCast<Int>()
	}

	override fun getAttribLocation(program: Int, name: String): Int = gl.getAttribLocation(program.get(), name)
	override fun getUniformLocation(program: Int, name: String): Int = run {
		val prg = program.get<WebGLProgram>().asDynamic(); if (prg.uniforms === undefined) prg.uniforms =
			js("({})"); if (prg.uniforms[name] === undefined) prg.uniforms[name] =
			gl.getUniformLocation(prg, name).alloc(); return prg.uniforms[name].unsafeCast<Int>()
	}

	override fun getBooleanv(pname: Int, data: KmlNativeBuffer): Unit =
		run { data.arrayInt[0] = gl.getParameter(pname).unsafeCast<Int>() }

	override fun getBufferParameteriv(target: Int, pname: Int, params: KmlNativeBuffer): Unit =
		run { params.arrayInt[0] = gl.getBufferParameter(target, pname).unsafeCast<Int>() }

	override fun getError(): Int = gl.getError()
	override fun getFloatv(pname: Int, data: KmlNativeBuffer): Unit =
		run { data.arrayFloat[0] = gl.getParameter(pname).unsafeCast<Float>() }
	...
	override fun getFramebufferAttachmentParameteriv(
		...
		run { params.arrayInt[0] = gl.getFramebufferAttachmentParameter(target, attachment, pname).unsafeCast<Int>() }
	...
	override fun getProgramInfoLog(
		...
		val str = gl.getProgramInfoLog(program.get()) ?: ""; length.arrayInt[0] =
			str.length; infoLog.putAsciiString(str)
	}

	override fun getRenderbufferParameteriv(target: Int, pname: Int, params: KmlNativeBuffer): Unit =
		run { params.arrayInt[0] = gl.getRenderbufferParameter(target, pname).unsafeCast<Int>() }
	...
	override fun getShaderInfoLog(shader: Int, bufSize: Int, length: KmlNativeBuffer, infoLog: KmlNativeBuffer): Unit =
		run {
			val str = gl.getShaderInfoLog(shader.get()) ?: ""; length.arrayInt[0] = str.length; infoLog.putAsciiString(
			str
		)
		}
	...
	override fun getShaderSource(shader: Int, bufSize: Int, length: KmlNativeBuffer, source: KmlNativeBuffer): Unit =
		run {
			val str = gl.getShaderSource(shader.get()) ?: ""; length.arrayInt[0] =
				str.length; source.putAsciiString(str)
		}
	...
	override fun getTexParameterfv(target: Int, pname: Int, params: KmlNativeBuffer): Unit =
		run { params.arrayFloat[0] = gl.getTexParameter(target, pname).unsafeCast<Float>() }

	override fun getTexParameteriv(target: Int, pname: Int, params: KmlNativeBuffer): Unit =
		run { params.arrayInt[0] = gl.getTexParameter(target, pname).unsafeCast<Int>() }

	override fun getUniformfv(program: Int, location: Int, params: KmlNativeBuffer): Unit =
		run { params.arrayFloat[0] = gl.getUniform(program.get(), location.get()).unsafeCast<Float>() }

	override fun getUniformiv(program: Int, location: Int, params: KmlNativeBuffer): Unit =
		run { params.arrayInt[0] = gl.getUniform(program.get(), location.get()).unsafeCast<Int>() }

	override fun getVertexAttribfv(index: Int, pname: Int, params: KmlNativeBuffer): Unit =
		run { params.arrayFloat[0] = gl.getVertexAttrib(index, pname).unsafeCast<Float>() }

	override fun getVertexAttribiv(index: Int, pname: Int, params: KmlNativeBuffer): Unit =
		run { params.arrayInt[0] = gl.getVertexAttrib(index, pname).unsafeCast<Int>() }

	override fun getVertexAttribPointerv(index: Int, pname: Int, pointer: KmlNativeBuffer): Unit =
		run { pointer.arrayInt[0] = gl.getVertexAttrib(index, pname).unsafeCast<Int>() }
	...
	): Unit = gl.readPixels(x, y, width, height, format, type, pixels.arrayUByte)
	...
	override fun shaderBinary(
		...
		length: Int
	): Unit = throw KmlGlException("shaderBinary not implemented in Webgl")

	override fun shaderSource(shader: Int, string: String): Unit =
		run { gl.shaderSource(shader.get(), "#ifdef GL_ES\nprecision mediump float;\n#endif\n$string") }
	...
	override fun texImage2D(
		...
		type: Int,
		pixels: KmlNativeBuffer
	): Unit = gl.texImage2D(target, level, internalformat, width, height, border, format, type, pixels.arrayUByte)
	...
	): Unit = gl.texImage2D(target, level, internalformat, format, type, (data as KmlImgNativeImageData).img)
	...
	override fun texParameterfv(target: Int, pname: Int, params: KmlNativeBuffer): Unit =
		gl.texParameterf(target, pname, params.arrayFloat[0])
	...
	override fun texParameteriv(target: Int, pname: Int, params: KmlNativeBuffer): Unit =
		gl.texParameteri(target, pname, params.arrayInt[0])
	...
	override fun texSubImage2D(
		...
		type: Int,
		pixels: KmlNativeBuffer
	): Unit = gl.texSubImage2D(target, level, xoffset, yoffset, width, height, format, type, pixels.arrayUByte)
	...
	override fun uniform1fv(location: Int, count: Int, value: KmlNativeBuffer): Unit =
		gl.uniform1fv(location.get(), value.arrayFloat)
	...
	override fun uniform1iv(location: Int, count: Int, value: KmlNativeBuffer): Unit =
		gl.uniform1iv(location.get(), value.arrayInt)
	...
	override fun uniform2fv(location: Int, count: Int, value: KmlNativeBuffer): Unit =
		gl.uniform2fv(location.get(), value.arrayFloat)
	...
	override fun vertexAttribPointer(
		...
		stride: Int,
		pointer: Int
	): Unit = gl.vertexAttribPointer(index, size, type, normalized, stride, pointer)
