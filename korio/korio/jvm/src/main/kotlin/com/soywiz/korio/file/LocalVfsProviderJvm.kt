...
class LocalVfsJvm : LocalVfs() {
	...
	override suspend fun watch(path: String, handler: (Vfs.FileEvent) -> Unit): Closeable {
		var running = true
			...
			while (running) {
				...
				for (e in key.pollEvents()) {
					...
					when (kind) {
						...
						StandardWatchEventKinds.ENTRY_CREATE -> {
							handler(Vfs.FileEvent(Vfs.FileEvent.Kind.CREATED, vfsFile))
						}
						StandardWatchEventKinds.ENTRY_MODIFY -> {
							handler(Vfs.FileEvent(Vfs.FileEvent.Kind.MODIFIED, vfsFile))
						}
						StandardWatchEventKinds.ENTRY_DELETE -> {
							handler(Vfs.FileEvent(Vfs.FileEvent.Kind.DELETED, vfsFile))
						}
					}
				}
				key.reset()
