...
abstract class Vfs {
	...
	abstract class Proxy : Vfs() {
		...
		suspend override fun <T : Any> readSpecial(path: String, clazz: KClass<T>): T =
			initOnce().access(path).readSpecial(clazz)

		suspend override fun put(path: String, content: AsyncInputStream, attributes: List<Attribute>) =
		...
		suspend override fun list(path: String) =
			asyncGenerate { initOnce(); for (it in access(path).list()) yield(transform(it)) }
