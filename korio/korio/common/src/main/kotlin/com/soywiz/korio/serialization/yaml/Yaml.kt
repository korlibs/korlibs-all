...
object Yaml {
	fun decode(@Lang("yaml") str: String) = read(ListReader(StrReader(str).tokenize()), level = 0)
	inline fun <reified T : Any> decodeToType(@Lang("yaml") s: String, mapper: ObjectMapper = Mapper): T =
		decodeToType(s, T::class, mapper)

	@Suppress("UNCHECKED_CAST")
	fun <T : Any> decodeToType(@Lang("yaml") s: String, clazz: KClass<T>, mapper: ObjectMapper = Mapper): T =
