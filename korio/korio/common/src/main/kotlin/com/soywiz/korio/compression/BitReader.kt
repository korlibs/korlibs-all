...
open class BitReader(val s: AsyncInputWithLengthStream) {
	...
	@PublishedApi internal var bitsavailable = 0
	...
	suspend inline fun readBits(bitcount: Int): Int {
		return if (this.bitsavailable >= bitcount) readBitsSure(bitcount) else ensureBits(bitcount).readBitsSure(bitcount)
	}
...
suspend fun BitReader.u32_be(): Int = u32_le().reverseBytes()
