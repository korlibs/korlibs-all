...
class SWFButtonCondAction {
	...
	private var actions = ArrayList<IAction>()
	...
	fun toString(indent: Int = 0, flags: Int = 0): String {
		...
		if ((flags and com.codeazur.as3swf.SWF.TOSTRING_FLAG_AVM1_BYTECODE) == 0) {
			...
			val context =
				ActionExecutionContext(actions, arrayListOf(), labelCount)
			for (i in 0 until actions.size) {
...
class SWFButtonRecord {
	...
	protected var filterList = ArrayList<IFilter>()

	fun parse(data: SWFData, level: Int = 1) {
	...
	fun toString(indent: Int = 0): String {
		var str = "Depth: $placeDepth, CharacterID: $characterId, States: "
...
class SWFClipActionRecord {
	lateinit var eventFlags: SWFClipEventFlags
	var keyCode: Int = 0
	...
	fun parse(data: SWFData, version: Int) {
...
class SWFClipActions {
	...
	protected var records = arrayListOf<SWFClipActionRecord>()
...
class SWFGlyphEntry {
	...
	fun parse(data: SWFData, glyphBits: Int, advanceBits: Int) {
		// GLYPHENTRYs are not byte aligned
		index = data.readUB(glyphBits)
		advance = data.readSB(advanceBits)
	}

	override fun toString(): String = "[SWFGlyphEntry] Index: " + index.toString() + ", Advance: " + advance.toString()
}
...
open class SWFGradient {
	...
	open fun parse(data: SWFData, level: Int) {
		data.resetBitsPending()
...
class SWFGradientRecord {
	...
	fun parse(data: SWFData, level: Int) {
		_level = level
		ratio = data.readUI8()
...
class SWFKerningRecord {
	...
	fun parse(data: SWFData, wideCodes: Boolean) {
...
open class SWFLineStyle {
	...
	override fun toString(): String =
		"[SWFLineStyle] Width: $width Color: " +
				(if (_level <= 2) ColorUtils.rgbToString(color) else ColorUtils.rgbaToString(color))
}
...
open class SWFMorphGradient {
	...
	protected var records = ArrayList<SWFMorphGradientRecord>()
...
class SWFRawTag {
	lateinit var header: SWFRecordHeader
...
open class SWFShape(var unitDivisor: Double = 20.0) {
	...
	protected fun readShapeRecords(data: SWFData, _fillBits: Int, _lineBits: Int, level: Int = 1) {
	...
	protected fun determineReferencePoint() {
	...
	open fun export(_handler: ShapeExporter) {
...
open class SWFShapeRecord {
	...
	@Suppress("unused")
	val isEdgeRecord: Boolean
		get() = (type == SWFShapeRecord.TYPE_STRAIGHTEDGE || type == SWFShapeRecord.TYPE_CURVEDEDGE)
...
data class SWFShapeRecordStraightEdge(
	...
	override fun toString(): String {
		...
		str += when {
			generalLineFlag -> "General: $deltaX,$deltaY"
			vertLineFlag -> "Vertical: $deltaY"
			else -> "Horizontal: $deltaX"
		}
		return str
	}
...
class SWFSoundInfo {
	...
	var envelopeRecords = ArrayList<SWFSoundEnvelope>()
...
class SWFTextRecord {
	...
	var glyphEntries = ArrayList<SWFGlyphEntry>()
