...
class DoubleLinkedList(private val debug: Boolean) : MutableCollection<Double> {
    //class LinkedList<Double>(private val debug: Boolean) {
    ...
    private var items = DoubleArray(16)
    ...
    operator override fun contains(item: Double) = indexOf(item) != NONE
    ...
    override fun addAll(elements: Collection<Double>): Boolean = true.apply { ensure(elements.size); for (e in elements) addLast(e) }
